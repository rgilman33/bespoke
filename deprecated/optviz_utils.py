...
def pixel_image(shape, sd):
    tensor = (torch.randn(*shape) * sd).to(device).requires_grad_(True)
    return [tensor], lambda: tensor

def image(h, w, c, sd=.01, batch=1, decorrelate=False, fft=False):
    shape = [batch, c, h, w]
    param_f = fft_image if fft else pixel_image #NOTE can also try fft here
    params, image_f = param_f(shape, sd)
    output = to_valid_rgb(image_f, decorrelate=decorrelate) #NOTE can also try decorr here, but be careful w channels
