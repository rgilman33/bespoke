...
_bce_loss_no_reduce = nn.BCELoss(reduction='none').cuda()

def bce_loss_no_reduce(targets, preds):
    l = _bce_loss_no_reduce(targets, preds)
    return l, l.mean()
...
def add_trajs_to_img(img, pred, targets, speed_mps=None):
    ...
    target_norm = TARGET_NORM[0,0,:] # squeezing what was unsqueezed
    trj = pred.detach().cpu().numpy() * target_norm
    trj_targets = targets.detach().cpu().numpy() * target_norm
...
class LossManager():
    def __init__(self):
        ...
        self.avg_losses = { # TODO should save and restore these rather than manually udpate
            "wp_angles": .005,
            "wp_curvatures": .07,
            "wp_headings": .01,
            "wp_rolls":.7, 
        ...
        self.loss_weights = {
            ...
            "lead_speed":.1,
            "td":.03,
            "pitch":.1, # calib only hits obsnet gradients
            "yaw":.1,
            "unc":.1,
        }
    def _step_loss(self, loss_name, loss):
        # update avg
        self.avg_losses[loss_name] = self.avg_losses[loss_name]*LOSS_EPS + loss.item()*(1-LOSS_EPS)
        weight = self.loss_weights[loss_name] * (self.avg_losses[self.baseline_loss_name] / self.avg_losses[loss_name])
        return weight*loss

    def step(self, losses_dict):
        total_loss = torch.HalfTensor([0]).to(device)
        for loss_name in losses_dict.keys():
            weighted_loss = self._step_loss(loss_name, losses_dict[loss_name])
...
def run_epoch(dataloader, #TODO prob put this in own file, it's a big one
    ...
    for update_counter in range(updates_per_epoch):
        ...
        if not batch: break
        (img, aux_model, aux_calib, wp_angles, wp_headings, wp_curvatures, wp_rolls, wp_zs, aux_targets, to_pred_mask), is_first_in_seq = batch
        ...
        _, rolls_loss = mse_loss_no_reduce(wp_rolls, wp_rolls_pred, mask=to_pred_mask, weights=loss_weights)
        _, zs_loss = mse_loss_no_reduce(wp_zs, wp_zs_pred, mask=to_pred_mask, weights=loss_weights)

        # aux targets
        approaching_stop_loss_no_reduce, approaching_stop_loss = mse_loss_no_reduce(aux_targets[:,:,0], sigmoid(aux_preds[:,:,0]))
        # approaching_stop_loss_no_reduce, approaching_stop_loss = bce_loss_no_reduce(aux_targets[:,:,0], sigmoid(aux_preds[:,:,0]))
        _, stop_dist_loss = mse_loss_no_reduce(aux_targets[:,:,1], aux_preds[:,:,1], mask=aux_targets[:,:,0]) # dist only care about when approaching stop
        
        has_lead_loss_no_reduce, has_lead_loss = mse_loss_no_reduce(aux_targets[:,:,3], sigmoid(aux_preds[:,:,3]))
        # has_lead_loss_no_reduce, has_lead_loss = bce_loss_no_reduce(aux_targets[:,:,3], sigmoid(aux_preds[:,:,3]))
        _, lead_dist_loss = mse_loss_no_reduce(aux_targets[:,:,4], aux_preds[:,:,4], mask=aux_targets[:,:,3])
        _, lead_speed_loss = mse_loss_no_reduce(aux_targets[:,:,5], aux_preds[:,:,5], mask=aux_targets[:,:,3])

        # calib loss
        pitch_loss = mse_loss(aux_calib[:,:,0], obs_net_out[:,:,0])
        yaw_loss = mse_loss(aux_calib[:,:,1], obs_net_out[:,:,1])
        unc_loss = mse_loss(torch.log(control_loss_no_reduce.mean(dim=-1).detach()), obs_net_out[:,:,2])

        # Vanilla steer cost. just for info
        ...
        # loss = loss_manager.step({
        #     "wp_angles": control_loss,
        #     "wp_curvatures": curvatures_loss,
        #     "wp_headings": headings_loss,
        #     "wp_rolls":rolls_loss,
        #     "wp_zs":zs_loss,
        #     "approaching_stop": approaching_stop_loss,
        #     "stop_dist": stop_dist_loss,
        #     "has_lead":has_lead_loss,
        #     "lead_dist":lead_dist_loss,
        #     "lead_speed":lead_speed_loss, 
        #     "td":torque_delta_loss,
        #     "pitch":pitch_loss,
        #     "yaw":yaw_loss,
        #     "unc":unc_loss,
        # })
        loss = control_loss*10 + curvatures_loss*.2 + headings_loss + rolls_loss*.01 + \
                approaching_stop_loss + stop_dist_loss + \
                has_lead_loss + lead_dist_loss + lead_speed_loss*.1 + \
                    unc_loss
        ...
        logger.log({f"lat_losses/{dataloader.path_stem}_control_loss": control_loss.item(),
                    ...
                    f"lat_losses/{dataloader.path_stem}_rolls_loss": rolls_loss.item(),   
                    f"lat_losses/{dataloader.path_stem}_zs_loss": zs_loss.item(),   
                    ...
                    f"lon_losses/{dataloader.path_stem}_lead_speed_loss": lead_speed_loss.item(),   
                    f"consistency losses/{dataloader.path_stem}_steer_cost":steer_cost.item(),
                    f"aux losses/{dataloader.path_stem}_unc_loss":unc_loss.item(),
        ...
        with torch.no_grad():
            ...
            if worst_control_loss > worst_control_loss_all:
                ...
                worst_control_loss_all_img = add_trajs_to_img(img[batch_worst_ix, bptt_worst_ix, :3, :, :], 
            ...
            if worst_approaching_stop_loss > worst_approaching_stop_loss_all:
                worst_approaching_stop_img = torch_to_np_img(img[batch_worst_ix, bptt_worst_ix, :3,:,:])
                a = aux_targets[batch_worst_ix, bptt_worst_ix,:].detach().cpu().numpy() * AUX_TARGET_NORM
                p = aux_preds[batch_worst_ix, bptt_worst_ix,:].detach().cpu().numpy() * AUX_TARGET_NORM
                worst_approaching_stop_img = draw_aux_targets(worst_approaching_stop_img, a, p)
            ...
            if worst_has_lead_loss > worst_has_lead_loss_all:
                worst_has_lead_img = torch_to_np_img(img[batch_worst_ix, bptt_worst_ix, :3,:,:])
                a = aux_targets[batch_worst_ix, bptt_worst_ix,:].detach().cpu().numpy() * AUX_TARGET_NORM
                p = aux_preds[batch_worst_ix, bptt_worst_ix,:].detach().cpu().numpy() * AUX_TARGET_NORM
                worst_has_lead_img = draw_aux_targets(worst_has_lead_img, a, p)
