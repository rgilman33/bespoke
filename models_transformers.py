...
class TransformCombine(nn.Module):
    ...
    def __init__(self, n_emb, seq_len):
        ...
        self.identity = torch.eye(n_emb, requires_grad=False).to("cuda")
    ...
    def _create_alphas_matrix(self, seq_len):
        base = torch.linspace(1,0, steps=seq_len)
        alphas_matrix = torch.zeros(seq_len, seq_len).to("cuda")
    ...
    def forward(self, x):
        ...
        r = torch.einsum('bsznm,bszmp->bsznp', x_exp, mixed_weights) # high mem use. Ideally we could repeat our mixed_weights rather than calc each sep, ie the diagonals of mixed weights matrix are the same, should reference same memory rather than create separately
        # bs, seq, seq, 1, emb
        ...
        attn = torch.ones(seq_len,seq_len).masked_fill(mask, -np.inf).to("cuda")
