...
class GPSTracker():
    ...
    def step(self, current_lat, current_lon_adjusted, current_speed_mps):
        # normal lat, ADJUSTED lon to get into 1:1 ratio
        ...
        if current_speed_mps > 2.0: # only update heading if moving faster than threshold
            ...
            prev_lat, prev_lon_adjusted = self.lats_history[-headings_estimation_ix_lookback], self.lons_history[-headings_estimation_ix_lookback]
            ...
            self.heading = np.arctan2(current_lat-prev_lat, current_lon_adjusted-prev_lon_adjusted)


        self.lats_history.append(current_lat)
        self.lons_history.append(current_lon_adjusted)
